...
func main() {
	...
	if err := client.Run(context.Background(), func(ctx context.Context) error {
		...
		listChannels, err := listChannelsFromFolders(ctx, raw, 4)
		if err != nil {
			return errors.Wrap(err, "list channels from folder")
		}

		for _, channel := range listChannels {
			if err := searchProductInChannel(ctx, raw, channel, "SSD"); err != nil {
...
func listChannelsFromFolders(ctx context.Context, raw *tg.Client, folderID int) ([]*tg.InputPeerChannel, error) {
	...
	if err != nil {
		return fmt.Errorf("erro ao buscar folders: %w", err), nil
	}
	...
	if folder == nil {
		return fmt.Errorf("❌ Pasta com ID %d não encontrada.", folderID), nil
	}
